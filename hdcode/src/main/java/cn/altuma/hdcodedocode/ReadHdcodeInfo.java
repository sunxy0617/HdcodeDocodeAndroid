...
class ReadHdcodeInfo {
    ...
    int codeWidth;
    byte[] pixs;//
    HdCodeRules hdCodeRules;

    public HdcodeInfo getHdcodeInfo(GrayImage binImage, String hdRulePath) {
        pixs = binImage.Pixs;
        codeWidth = binImage.Width;


        hdCodeRules = new HdCodeRules(hdRulePath);
    ...
    private byte[] readPlans(long planNum) {
        return hdCodeRules.getPlan(planNum);
    }
    ...
    private int[][] readPlan(byte[] rules, int index)// 将数据库字节翻译为图片并显示
    {
        ...
        if (rules != null) {
            ...
            for (int i = 65536 * index, j = 0; j < 65536; i++, j++) {
                pixs[p >> 8][p & 255] = ((rules[i >> 3] >> (7 - i % 8)) & 0x1) * 254 + 1;
